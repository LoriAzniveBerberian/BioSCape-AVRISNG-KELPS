...
Read BioSCape AVIRIS-NG L2A and L2B RFL ORT NetCDF files. Build bbox
footprints, intersect with coastline, stage selected files, export
quicklooks (RGB + NDVI) as GeoTIFFs, and extract per-pixel kelp spectra.

NetCDF structure (L2A):
  - variables: easting, northing, transverse_mercator
  - group: reflectance (reflectance, wavelength)

NetCDF structure (L2B):
  - variables: projection (with GeoTransform attribute)
  - dimensions: easting, northing (no coordinate variables)
  - group: reflectance (reflectance, wavelength)
  - group: masks (topo, brdf, glint correction masks)
...
def _find_gridmap_var(ds: nc.Dataset) -> Optional[str]:
    for name in (GRIDMAP_VAR, GRIDMAP_VAR_L2B):
...
def get_crs_from_grid_mapping(ds: nc.Dataset) -> Tuple[Optional[CRS], Optional[int]]:
    """Best-effort CRS from the grid mapping variable.
    Checks for transverse_mercator (L2A) or projection (L2B)."""
    gm_name = _find_gridmap_var(ds)
    if gm_name is None:
        return None, None

    gm = ds.variables[gm_name]
...
def _xy_from_geotransform(ds: nc.Dataset, n_cols: int, n_rows: int) -> Tuple[np.ndarray, np.ndarray]:
    """Reconstruct easting/northing pixel-center arrays from GeoTransform."""
    for name in (GRIDMAP_VAR, GRIDMAP_VAR_L2B):
        if name in ds.variables and "GeoTransform" in ds.variables[name].ncattrs():
            gt = [float(v) for v in ds.variables[name].getncattr("GeoTransform").split()]
            x0, dx, _, y0, _, dy = gt
            x = x0 + (np.arange(n_cols) + 0.5) * dx
            y = y0 + (np.arange(n_rows) + 0.5) * dy
            return x, y
    raise ValueError("no GeoTransform found -- cannot reconstruct coordinates")
...
def load_spec(ds: nc.Dataset) -> CubeSpec:
    ...
    if X_VAR_NAME in ds.variables and Y_VAR_NAME in ds.variables:
        x = np.array(ds.variables[X_VAR_NAME][:]).astype(float)
        y = np.array(ds.variables[Y_VAR_NAME][:]).astype(float)
    else:
        x, y = _xy_from_geotransform(ds, shape[x_axis], shape[y_axis])
